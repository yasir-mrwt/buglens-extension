...
function createInputBatcher(delayMs, emit) {
  let timer = null;
...
function classifyNetworkUrl(url, pageOrigin) {
...
(() => {
  ...
  const MAX_UI_NODES_DEFAULT = 4e3;
  const MAX_SELECTED_TEXT_LENGTH = 5e3;
  const MAX_VISIBLE_SUMMARY_LENGTH = 3e3;
  const MAX_AGENT_ELEMENTS = 80;
  const AGENT_TYPE_DELAY_MS = 38;
  const ALLOWED_AGENT_ACTIONS = /* @__PURE__ */ new Set(["click", "type", "clear", "select", "check", "uncheck", "scroll", "highlight", "wait", "observe", "navigate"]);
  ...
  const inputBatcher = createInputBatcher(300, (target) => {
    ...
    emitObservation("user-action", {
      ...
      valueLength: element.value !== void 0 && element.value !== null ? String(element.value).length : 0
    });
  });
  class ObservationLayer {
    ...
    handleClick(event) {
      const target = event.target && event.target.closest ? event.target.closest('button, a, input, select, textarea, [role="button"], [onclick]') : event.target;
      if (!target) return;
    ...
    describeMutation(record) {
      ...
      const descriptor = {
        ...
        textLength: record.type === "characterData" && record.target && record.target.textContent !== void 0 ? String(record.target.textContent).length : 0
      };
      return descriptor;
    }
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.type === "TESTPILOT_RUN_UI_SCAN") {
      ...
      runWhenIdle(() => runUiScan(settings)).then((scan) => sendResponse({ ok: true, scan })).catch((error) => sendResponse({
        ok: false,
    ...
    if (message.type === "TESTPILOT_RUN_AGENT") {
      runWhenIdle(() => runAgentWorkflow(message.command || "", message.options || {})).then((result) => sendResponse({ ok: true, result })).catch((error) => sendResponse({
        ok: false,
  ...
  async function runAgentWorkflow(command, options = {}) {
    const userCommand = sanitizePageText(command, 1e3);
    ...
    const resumeFromStepIndex = approvalValid ? Math.min(Math.max(0, Number(options.approval.resumeFromStepIndex || 0)), plan.steps.length) : 0;
    ...
    const actionResults = approvalValid ? sanitizePreviousAgentActionResults(options.approval.previousActionResults) : [];
    if (safety.approved) {
      emitAgentEvent("approval-granted", {
        requestKey,
        taskType,
        summary: resumeFromStepIndex > 0 ? `Tester approved once. Resuming at step ${resumeFromStepIndex + 1}.` : "Tester approved the preflighted serious action once. Continuing safely."
      });
    }
    if (!safety.ok) {
      const result2 = evaluateAgentResult(plan, actionResults, observeAgentPage(), safety);
    ...
    const observation = observeAgentPage();
  ...
  function describeAgentElement(element, index) {
    ...
    return {
      ...
      visible: true,
      selectorHint,
      href: tag === "a" ? sanitizePageText(element.getAttribute("href") || "", 240) : void 0
    };
  }
  ...
  function getImportantVisibleText() {
    return Array.from(document.querySelectorAll('main, h1, h2, h3, [role="main"], [role="alert"], [role="status"], .error, .success, .message')).filter((element) => isVisibleElement(element, true)).map((element) => sanitizePageText(getShortText(element), 180)).filter(Boolean).slice(0, 8);
  }
  ...
  function buildAgentPlan(command, taskType, context, options = {}) {
    ...
    const formFields = () => primaryForm ? primaryForm.fields.map((index) => elements.find((item) => item.index === index)).filter(Boolean) : elements.filter((item) => ["input", "textarea", "select", "checkbox", "radio"].includes(item.role) && !item.disabled).slice(0, 6);
    const submitButton = () => (primaryForm ? primaryForm.submitButtons : []).map((index) => elements.find((item) => item.index === index)).find(Boolean) || first((item) => item.role === "button" && /submit|log in|login|sign in|sign up|send|continue|save|search|place/i.test(`${item.text || ""} ${item.label || ""}`));
  ...
  function normalizeStructuredAgentPlan(candidatePlan, fallbackPlan, taskType, context) {
    ...
    const normalizedSteps = candidatePlan.steps.slice(0, 12).map((step) => normalizeStructuredAgentStep(step, availableTargets)).filter(Boolean);
    if (!normalizedSteps.length) return fallbackPlan;
    const targetElements = normalizedSteps.map((step) => step.targetIndex).filter(Boolean);
    return {
  ...
  function normalizeStructuredAgentStep(step, availableTargets) {
    ...
    return {
      action,
      targetIndex: Number.isFinite(targetIndex) && targetIndex > 0 ? targetIndex : void 0,
      value: sanitizeStructuredStepValue(action, step.value),
      url: action === "navigate" ? sanitizePageText(step.url || "", 300) : void 0,
  ...
  function sanitizeStructuredStepValue(action, value) {
    if (action === "wait" || action === "scroll") return Math.min(3e3, Math.max(0, Number(value) || 0));
  ...
  function validateAgentPlan(plan, context, approval = {}, requestKey = "") {
    ...
    const unresolvedConfirmation = approvalGranted ? confirmations.filter((item) => item.fingerprint !== approval.stepFingerprint).map((item) => item.reason) : confirmations.map((item) => item.reason);
    return {
      ok: blocked.length === 0,
      blocked,
  ...
  function isAgentApprovalTokenValid(approval, requestKey) {
    return Boolean(approval && approval.approved === true && approval.requestKey && String(approval.requestKey) === String(requestKey || "") && typeof approval.stepFingerprint === "string" && approval.stepFingerprint);
  }
  ...
  function getAgentStepConfirmation(step, context) {
    const target = step.targetIndex ? (context.elements || []).find((item) => item.index === step.targetIndex) : null;
    const text = `${(target == null ? void 0 : target.text) || ""} ${(target == null ? void 0 : target.label) || ""} ${step.value || ""} ${step.url || ""}`.toLowerCase();
    const targetLabel = (target == null ? void 0 : target.text) || (target == null ? void 0 : target.label) || (target == null ? void 0 : target.placeholder) || step.action;
    ...
    if (isAgentFormSubmitAction(step, target, context)) {
      ...
      return {
        reason: `Form submit/continue requires approval for target #${(target == null ? void 0 : target.index) || "unknown"}: ${targetLabel}`,
        category: destructive ? "high-risk-action" : "form-submit",
        rememberable: !destructive,
        targetLabel
      };
    }
    if (["click", "navigate", "select", "check", "uncheck"].includes(step.action) && /delete|remove|destroy|logout|log out|sign out|reset password|payment|checkout|pay now|purchase|subscribe|bank|billing|create account|register|sign up|signup|save changes|confirm order|place order/.test(text)) {
      const destructive = isHighRiskAgentActionText(text);
      return {
        reason: `Serious action requires approval for target #${(target == null ? void 0 : target.index) || "unknown"}: ${targetLabel}`,
        category: destructive ? "high-risk-action" : "form-submit",
        rememberable: !destructive,
        targetLabel
      };
  ...
  function getAgentStepApprovalFingerprint(step, context, confirmation = null) {
    ...
    return simpleHash(JSON.stringify({
      action: (step == null ? void 0 : step.action) || "",
      targetIndex: (step == null ? void 0 : step.targetIndex) || null,
      targetLabel: resolvedConfirmation.targetLabel || (target == null ? void 0 : target.label) || (target == null ? void 0 : target.text) || "",
      category: resolvedConfirmation.category || "",
      url: (step == null ? void 0 : step.url) || ""
    }));
  }
  async function executeAgentStep(step, stepIndex, context, requestKey) {
    ...
    try {
      ...
      if (step.action === "select") {
        highlightAgentElement(element, 1e3);
      ...
      if (step.action === "click") {
        highlightAgentElement(element, 900);
        (_a = element.scrollIntoView) == null ? void 0 : _a.call(element, { block: "center", inline: "center" });
  ...
  function observeAgentPage(context) {
    ...
    const dialogs = Array.from(document.querySelectorAll('[role="dialog"], dialog, [aria-modal="true"]')).filter((element) => isVisibleElement(element, true)).map((element) => sanitizePageText(getShortText(element), 160)).filter(Boolean).slice(0, 5);
    const tables = Array.from(document.querySelectorAll('table, [role="table"]')).filter((element) => isVisibleElement(element, true)).map((table) => `${table.querySelectorAll ? table.querySelectorAll('tr, [role="row"]').length : 0} visible row(s)`).slice(0, 3);
  ...
  function getResultCountSignals() {
    ...
    return Array.from(document.querySelectorAll(selector)).filter((element) => isVisibleElement(element, true)).map((element) => {
  ...
  function getButtonStateSignals() {
    return Array.from(document.querySelectorAll('button, input[type="submit"], input[type="button"], [role="button"]')).filter((element) => isVisibleElement(element, true)).slice(0, 12).map((element) => {
  ...
  function evaluateAgentResult(plan, actionResults, observation, safety) {
    if (safety && !safety.ok) {
      return {
        ...
        summary: safety.requiresConfirmation ? "Agent stopped before execution because the plan needs tester confirmation." : "Agent stopped before execution because the plan was unsafe or invalid.",
        passedChecks: [],
    ...
    const evidence = [...(observation == null ? void 0 : observation.evidence) || [], ...actionResults.map((item) => `${item.action}: ${item.message}`)].slice(0, 12);
    const hasVisibleValidation = Boolean(
      observation.fieldValidation && observation.fieldValidation.length || observation.visibleAlerts && observation.visibleAlerts.length
    );
    const hasInteractionEvidence = Boolean(
      hasVisibleValidation || observation.visibleDialogs && observation.visibleDialogs.length || observation.resultSignals && observation.resultSignals.length || observation.tableSignals && observation.tableSignals.length || observation.buttonSignals && observation.buttonSignals.length
    );
    let status = failures.length ? "failed" : "needs_review";
    let summary = failures.length ? `${failures.length} agent step${failures.length === 1 ? "" : "s"} failed and needs review.` : `Agent completed ${successCount} step${successCount === 1 ? "" : "s"}; review the observations before filing.`;
    if (plan.taskType === "highlight_interactions" && !failures.length) {
      ...
      summary = highlighted ? `Highlighted ${highlighted} visible interactive element${highlighted === 1 ? "" : "s"} without typing or clicking.` : "No visible interactive elements were available to highlight.";
      ...
      summary = observation.accessibility.length ? `${observation.accessibility.length} accessibility observation${observation.accessibility.length === 1 ? "" : "s"} found.` : "No basic visible accessibility issues were detected by the agent.";
      ...
      summary = hasInteractionEvidence ? "Agent executed the interaction and captured visible evidence. Confirm expected behavior before filing." : "Agent executed the interaction, but visible evidence was weak. Manual review is required.";
  ...
  function realDataDisclaimer(dataStrategy) {
    return dataStrategy === "real_user_provided" ? "Use safe realistic dummy QA data instead of private real user credentials." : "Enter valid-looking dummy QA data requested by the prompt.";
  }
  ...
  function setElementValue(element, value) {
    ...
    const prototype = element.tagName === "TEXTAREA" ? window.HTMLTextAreaElement && window.HTMLTextAreaElement.prototype : window.HTMLInputElement && window.HTMLInputElement.prototype;
  ...
  async function setElementValueAnimated(element, value, meta = {}) {
    ...
    if (!element) return;
    (_a = element.scrollIntoView) == null ? void 0 : _a.call(element, { block: "center", inline: "center" });
    (_b = element.focus) == null ? void 0 : _b.call(element);
    setElementValue(element, "");
  ...
  function maskAgentTypedValue(value, target) {
    const text = String(value || "");
    const hint = `${(target == null ? void 0 : target.type) || ""} ${(target == null ? void 0 : target.label) || ""} ${(target == null ? void 0 : target.placeholder) || ""}`.toLowerCase();
  ...
  function selectSafeOption(element, requestedValue) {
    ...
    const next = requestedValue && requestedValue !== "__FIRST_SAFE_OPTION__" ? options.find((option) => option.value === requestedValue && !option.disabled) : options.find((option) => !option.disabled && String(option.value || option.textContent || "").trim());
  ...
  function getVisibleHeadings() {
    return Array.from(document.querySelectorAll('h1, h2, h3, [role="heading"]')).filter((element) => isVisibleElement(element, true)).map((element) => sanitizePageText(getShortText(element), 160)).filter(Boolean).slice(0, 8);
  }
  function getVisibleAlertTexts() {
    ...
    return Array.from(document.querySelectorAll(selector)).filter((element) => isVisibleElement(element, true)).map((element) => sanitizePageText(getShortText(element), 220)).filter(Boolean).slice(0, 10);
  }
  ...
  function runUiScan(settings) {
    ...
    for (const el of elements) {
      ...
      const isInteractive = el.matches ? el.matches('button, a[href], input, select, textarea, [role="button"], [onclick]') : false;
      const isMeaningfulElement = rect.width >= 24 && (isInteractive || hasDirectText(el) || !["span", "i", "b", "small"].includes(tag));
      const isTransformed = Boolean(style.transform && style.transform !== "none");
      if (overflowAmount > minimumOverflow && isInVerticalViewport && isMeaningfulElement && !isTransformed) {
        addIssue(
      ...
      if (isTextClippingCandidate(el, style, rect)) {
        ...
        const meaningfulOverflow = widthOverflow > Math.max(12, el.clientWidth * 0.05) || heightOverflow > Math.max(8, el.clientHeight * 0.15);
      ...
      if ((style.position === "fixed" || style.position === "sticky") && rect.width >= window.innerWidth * 0.8 && rect.height >= window.innerHeight * 0.4 && style.pointerEvents !== "none" && Number(style.opacity || 1) > 0.2 && Number.isFinite(zIndex) && zIndex >= 100 && hasForegroundContent) {
        addIssue(
          "large-fixed-overlay",
          "medium",
  ...
  function getElementLabel(element) {
    if (!element || !element.getAttribute) return "";
    return element.getAttribute("aria-label") || element.getAttribute("title") || element.getAttribute("name") || element.getAttribute("id") || element.tagName.toLowerCase();
  }
  ...
  function groupButtonStyles(maxNodes) {
    ...
    const buttons = Array.from(document.querySelectorAll('button, input[type="button"], input[type="submit"], [role="button"]')).slice(0, maxNodes).filter((element) => isVisibleElement(element, true));
    for (const button of buttons) {
      ...
      if (!groups.has(key)) groups.set(key, { key, count: 0, signatures: /* @__PURE__ */ new Map() });
    ...
    return Array.from(groups.values()).filter((group) => group.count >= 2 && group.signatures.size > 1).map((group) => ({ key: group.key, count: group.count, variants: Array.from(group.signatures.values()) }));
  }
  ...
  function isDecorativeElement(el, style) {
    ...
    const zIndex = Number.parseInt(style.zIndex, 10);
    return el.tagName.toLowerCase() === "canvas" || style.pointerEvents === "none" || Number.isFinite(zIndex) && zIndex < 0 || Number(style.opacity || 1) < 0.15 || decorativeName || !hasDirectText(el) && !hasInteractiveChild && ["fixed", "absolute"].includes(style.position);
  }
  function isTextClippingCandidate(el, style, rect) {
    const textTags = /* @__PURE__ */ new Set(["p", "span", "a", "button", "label", "li", "td", "th", "h1", "h2", "h3", "h4", "h5", "h6"]);
    const tag = el.tagName.toLowerCase();
    ...
    return style.overflow === "hidden" || style.overflow === "clip" || style.overflowX === "hidden" || style.overflowY === "hidden" || style.whiteSpace === "nowrap";
  }
  ...
  function getSelector(el) {
    ...
    while (current && current.nodeType === Node.ELEMENT_NODE && path.length < 3) {
      ...
      const stableClasses = Array.from(current.classList || []).filter(isStableSelectorClass).slice(0, 2);
